...
if (!interactive()) {
    ...
    parser <- setup_default_argparser(
        description = "Combine downsampling runs CellChat per sample"
    )
    ...
    args$input_dir <- glue("output/test_pipeline/300_postproc_cellchat")
...
obj_list <- list.files(args$input_dir,
    full.names = TRUE,
    pattern = glue("cellchat__{args$sample_id}__run__\\d__postproc\\.rds")
)
...
all_obj <- all_obj %>%
    mutate(
        # https://github.com/sqjin/CellChat/issues/353#issuecomment-1381772161
        # P-values of 0 means none of the permutations have higher interaction probability than the observed data (i.e., the original scRNA-seq data)
    ...
    reframe(
        ...
        interaction_score = ifelse(n() == args$n_repeats, mean(interaction_score), NA),
        n_detected = n()
    )
head(all_obj)
log_info("Save RDS...")
saveRDS(all_obj, glue("{args$output_dir}/cellchat__{args$sample_id}__postproc.rds"))
