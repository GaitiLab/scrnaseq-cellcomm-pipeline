...
if (!interactive()) {
    ...
    parser <- setup_default_argparser(
        description = "Combine downsampling runs Cell2Cell per sample"
    )
    ...
    args <- list()
    args$log_level <- 5
    args$output_dir <- glue("{here::here()}/output/test_pipeline/302_postproc_cell2cell")
    args$input_dir <- glue("{here::here()}/output/test_pipeline/302_postproc_cell2cell")
...
obj_list <- list.files(args$input_dir,
    full.names = TRUE,
    pattern = glue("cell2cell__{args$sample_id}__run__\\d__postproc\\.rds")
)
...
head(all_obj)
log_info("Save RDS...")
saveRDS(all_obj, glue("{args$output_dir}/cell2cell__{args$sample_id}__postproc.rds"))
log_info("COMPLETED!")
