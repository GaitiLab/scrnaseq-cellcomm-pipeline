...
if (!interactive()) {
    ...
    parser <- setup_default_argparser(
        description = "Combine downsampling runs LIANA per sample"
    )
    ...
    args <- list()
    args$log_level <- 5
    args$output_dir <- glue("{here::here()}/output/test_pipeline/301_postproc_liana")
    args$input_dir <- glue("{here::here()}/output/test_pipeline/301_postproc_liana")
...
obj_list <- list.files(args$input_dir,
    full.names = TRUE,
    pattern = glue("liana__{args$sample_id}__run__\\d__postproc\\.rds")
)
...
head(all_obj)
log_info("Save RDS...")
saveRDS(all_obj, glue("{args$output_dir}/liana__{args$sample_id}__postproc.rds"))
log_info("COMPLETED!")
