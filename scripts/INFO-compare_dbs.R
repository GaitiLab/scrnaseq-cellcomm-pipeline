...
cellchat_db <- readRDS(args$cellchat_db) %>% mutate(
    method = "CellChat extracted",
    interaction = glue("{source_genesymbol} - {target_genesymbol}")
)
cpdb_db <- readRDS(args$cpdb_db) %>% mutate(method = "CellphoneDB extracted", interaction = glue("{source_genesymbol} - {target_genesymbol}"))
liana_db_base <- readRDS(args$liana_db) %>% mutate(interaction = glue("{source_genesymbol} - {target_genesymbol}"))

interactions <- list(
    cellchat_db = cellchat_db %>% pull(interaction),
    cpdb_db = cpdb_db %>% pull(interaction),
    liana_db_consensus = liana_db_base %>% filter(method == "LIANA consensus") %>% pull(interaction),
    liana_db_rami = liana_db_base %>% filter(method == "LIANA Ramilowski2015") %>% pull(interaction)
)
