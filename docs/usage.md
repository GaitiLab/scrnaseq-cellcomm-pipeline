# GaitiLab/scrnaseq-cellcomm-pipeline: Usage
...
```bash
nextflow run GaitiLab/scrnaseq-cellcomm-pipeline -profile docker -params-file params.yaml
```
...
First, go to the [GaitiLab/scrnaseq-cellcomm-pipeline releases page](https://github.com/GaitiLab/scrnaseq-cellcomm-pipeline/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
