...
# Copy files
cp -R -u -p  "${cellchat_db}" "${target_interactions_db_dir}"
cp -R -u -p  "${liana_db}" "${target_interactions_db_dir}"
cp -R -u -p  "${liana_db_csv}" "${target_interactions_db_dir}"
cp -R -u -p  "${ref_db}" "${target_interactions_db_dir}"
cp -R -u -p  "${cellphone_db}" "${target_interactions_db_dir}"
